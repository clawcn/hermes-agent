...
def _managed_gateway_available(nous_auth_present: bool) -> bool:
    """Managed gateways are available whenever a Nous auth session exists."""
    return bool(nous_auth_present)
...
def get_nous_subscription_features(
    ...
    browser_provider = (
        str(config.get("browser", {}).get("cloud_provider") or "local").strip().lower()
    ...
    direct_openai_tts = bool(get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"))
    direct_elevenlabs = bool(get_env_value("ELEVENLABS_API_KEY"))
    direct_browserbase = bool(get_env_value("BROWSERBASE_API_KEY") and get_env_value("BROWSERBASE_PROJECT_ID"))
    direct_browser_use = bool(get_env_value("BROWSER_USE_API_KEY"))
    direct_modal = bool(get_env_value("MODAL_TOKEN_ID") and get_env_value("MODAL_TOKEN_SECRET"))

    managed_web_available = _managed_gateway_available(nous_auth_present)
    managed_image_available = _managed_gateway_available(nous_auth_present)
    managed_tts_available = _managed_gateway_available(nous_auth_present)
    managed_browser_available = _managed_gateway_available(nous_auth_present)
    managed_modal_available = _managed_gateway_available(nous_auth_present)
    ...
    modal_managed = (
        ...
        and modal_tool_enabled
        and managed_modal_available
        and not direct_modal
    )
    modal_available = bool(terminal_backend != "modal" or managed_modal_available or direct_modal)
    modal_active = bool(
        modal_tool_enabled
        and (
            terminal_backend != "modal"
            or direct_modal
            or managed_modal_available
        )
    )
    ...
    features = {
        ...
        "modal": NousFeatureState(
            ...
            direct_override=terminal_backend == "modal" and direct_modal,
